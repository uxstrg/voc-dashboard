...
export function getStatusColor(score: number): string {
  ...
  return STATUS_COLORS[label] || '#8A9980';
}

// Gap 상태 컬러
export function getGapStatusColor(gap: number): string {
  if (gap > 5) return GAP_STATUS_COLORS['기대 초과'];
  if (gap >= -5) return GAP_STATUS_COLORS['기대 충족'];
  return GAP_STATUS_COLORS['기대 이하'];
}

// Gap 표시 라벨 (1.8항: 기대 초과 +N / 기대 충족 / 기대 이하 -N)
export function getGapDisplayLabel(gap: number): string {
  if (gap > 5) return `기대 초과 +${gap}`;
...
export function calcChannelSentiment(vocData: { source: string; platform?: string; issues: DiagnosisIssue[] }[]) {
  return PLATFORMS.map((platform) => {
    const items = vocData.filter(v => ((v as any).platform ?? v.source) === platform);
